...
const ResultView: React.FC<ResultViewProps> = ({ originalImage, transformedImages, onReset, onRetryItem, onDownloadAll, isDownloadingAll }) => {
  const isAnyItemLoading = transformedImages.some(item => item.isLoading);
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {transformedImages.map((item, index) => (
                <div key={index} className="bg-gray-800/70 p-3 rounded-lg flex flex-col shadow-md border border-gray-700">
                  ...
                  <p className="text-sm font-medium text-center text-gray-300 flex-grow mb-2 min-h-[2.5rem] flex items-center justify-center">{item.name}</p>
                  <div className="mt-auto flex items-center space-x-2">
                    <a
                      href={item.imageUrl}
                      download={`${item.name.replace(/\s+/g, '_').toLowerCase()}.png`}
                      onClick={(e) => { if (item.isLoading) e.preventDefault(); }}
                      className={`flex-1 text-center bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-3 rounded-md transition-all duration-300 text-xs flex items-center justify-center space-x-1.5 ${item.isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                    ...
                    <button
                        ...
                        className="flex-1 text-center bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-3 rounded-md transition-all duration-300 text-xs flex items-center justify-center space-x-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
